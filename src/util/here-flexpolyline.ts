 ...
 * 
 * https://raw.githubusercontent.com/heremaps/flexible-polyline/master/javascript/index.js
 */
...
export function flexPolylineToLineString(encoded : any) : Array<Array<number>> {

    let shape = flex_polyline_decode(encoded)

    let result = []

    for (let coords of shape.polyline) {
...
export function flex_polyline_decode(encoded: any) {
...
function decodeUnsignedValues(encoded: any) {
    let result: any = Num(0);
    let shift: any = Num(0);
    const resList: any = [];

    encoded.split('').forEach((char: any) => {
        const value: any = Num(decodeChar(char));
        //@ts-ignore
