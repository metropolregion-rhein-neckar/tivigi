...
export function quantilesTimeSeriesStyleFactory(source: ol_source.Vector, attributeName: string, timestamp: string, styleConfig: any = {}): StyleAndLegend {
    ...
    const legend = makeQuantilesLegend(quantiles, colors, noDataColor, lineColor, lineWidth)
    ...
    return new StyleAndLegend(quantilesStyleFunction, legend)
}
...
function computeQuantiles(source: ol_source.Vector, attributeName: string, timestamp: string, numQuantiles: number): Array<number> {
    ...
    for (let f of source.getFeatures()) {
        ...
        //let attribute = f.get(attributeName)
        let attribute = props["entity"][attributeName]
