# Table of Contents
  ...
  - 4.) [The Tivigi philosophy](#4-the-tivigi-philosophy)
    - 4.1.) [Tivigi is *not* a configurable out-of-the-box application.](#41-tivigi-is-not-a-configurable-out-of-the-box-application)
    - 4.2.) [Tivigi does *not* follow a "core and modules" architecture.](#42-tivigi-does-not-follow-a-core-and-modules-architecture)
    - 4.3.) [Tivigi is front-end / browser only](#43-tivigi-is-front-end--browser-only)
    - 4.4.) [Tivigi makes heavy use of component reuse and composition](#44-tivigi-makes-heavy-use-of-component-reuse-and-composition)
    - 4.5.) [Tivigi is strongly focused on declarative (i.e. Vue.js template based, HTML-like) coding](#45-tivigi-is-strongly-focused-on-declarative-ie-vuejs-template-based-html-like-coding)
  - 5.) [Getting Started](#5-getting-started)
    - 5.1.) [What do I *need* to know?](#51-what-do-i-need-to-know)
    - 5.2.) [What would be *valuable* to know?](#52-what-would-be-valuable-to-know)
    - 5.3.) [About single-page JavaScript applications](#53-about-single-page-javascript-applications)
    - 5.4.) [About Vue.js](#54-about-vuejs)
      - 5.4.1.) [What are Vue.js components?](#541-what-are-vuejs-components)
      - 5.4.2.) [What is Vue.js reactivity?](#542-what-is-vuejs-reactivity)
    - 5.5.) [About TypeScript](#55-about-typescript)
    - 5.6.) [About toolchain complexity](#56-about-toolchain-complexity)
    - 5.7.) [Setting up the first sample project](#57-setting-up-the-first-sample-project)
      - 5.7.1.) [Installed software prerequisites](#571-installed-software-prerequisites)
      - 5.7.2.) [Cloning the GitHub repository](#572-cloning-the-github-repository)
      - 5.7.3.) [Downloading the dependencies with npm](#573-downloading-the-dependencies-with-npm)
    - 5.8.) [A look into the Tivigi source folder](#58-a-look-into-the-tivigi-source-folder)
    - 5.9.) [Running the sample application](#59-running-the-sample-application)
  - 6.) [Diving into the code](#6-diving-into-the-code)
    - 6.1.) [The root component and its template file](#61-the-root-component-and-its-template-file)
    - 6.2.) [A side note about the index.html file](#62-a-side-note-about-the-indexhtml-file)
    - 6.3.) [Parts of a Vue.js component](#63-parts-of-a-vuejs-component)
    - 6.4.) [Declarative vs imperative coding](#64-declarative-vs-imperative-coding)
    - 6.5.) [Template files vs. TypeScript files](#65-template-files-vs-typescript-files)
  - 7.) [Tutorial Step 1: My first map](#7-tutorial-step-1-my-first-map)
    - 7.1.) [Reactive HTML attributes](#71-reactive-html-attributes)
    - 7.2.) [Roles of the components in the first example](#72-roles-of-the-components-in-the-first-example)
    - 7.3.) [About "data components" in general](#73-about-data-components-in-general)
    - 7.4.) [The DataMap component](#74-the-datamap-component)
    - 7.5.) [The MapPanel component](#75-the-mappanel-component)
  ...
  - 9.) [Tutorial Step 3: Adding a layer tree](#9-tutorial-step-3-adding-a-layer-tree)
    - 9.1.) [The `<LayerTree>` component](#91-the-layertree-component)
    - 9.2.) [The `<Modal>` component](#92-the-modal-component)
    - 9.3.) [The `<Superbutton>` component](#93-the-superbutton-component)
    - 9.4.) [Bidireactional data binding with `v-model` and `.sync`](#94-bidireactional-data-binding-with-v-model-and-sync)
  - 10.) [Tutorial Step 4: Adding a legend (list of active layers) and feature info tool](#10-tutorial-step-4-adding-a-legend-list-of-active-layers-and-feature-info-tool)
    - 10.1.) [The `<ActiveLayers>` component](#101-the-activelayers-component)
    - 10.2.) [The `<FeatureInfoTool>` component](#102-the-featureinfotool-component)
  - 11.) [Tutorial Step 5: Adding more features](#11-tutorial-step-5-adding-more-features)
    - 11.1.) [The `<LocationSearch>` component](#111-the-locationsearch-component)
    - 11.2.) [The `<MouseCoordinatesTool>` component](#112-the-mousecoordinatestool-component)
    - 11.3.) [The `<SetMapExtentTool>` component](#113-the-setmapextenttool-component)
  - 12.) [Troubleshooting](#12-troubleshooting)
    - 12.1.) [Problem: First setting of previously undefined variables in templates trigger component reload](#121-problem-first-setting-of-previously-undefined-variables-in-templates-trigger-component-reload)
...
## 4.1. Tivigi is *not* a configurable out-of-the-box application.
...
## 4.2. Tivigi does *not* follow a "core and modules" architecture.
...
## 4.3. Tivigi is front-end / browser only
...
## 4.4. Tivigi makes heavy use of component reuse and composition
...
## 4.5. Tivigi is strongly focused on declarative (i.e. Vue.js template based, HTML-like) coding
...
## 5.2. What would be *valuable* to know?
...
### 5.4.1. What are Vue.js components?
...
### 5.4.2. What is Vue.js reactivity?
...
## 5.6. About toolchain complexity
...
## 5.7. Setting up the first sample project

### 5.7.1. Installed software prerequisites
...
## 5.8. A look into the Tivigi source folder
...
## 5.9. Running the sample application
...
## 6.1. The root component and its template file
...
## 6.2. A side note about the index.html file
...
## 6.3. Parts of a Vue.js component
...
## 6.4. Declarative vs imperative coding
...
## 6.5. Template files vs. TypeScript files
...
# 7. Tutorial Step 1: My first map
...
As an example: The expression `<MapPanel :map="local.map"/>` says that the component instance's member variable `this.local.map` (actually the member variable `map` of the the component's member variable `local`, i.e. a two levels deep nesting hierarchy) should be passed as the value of the `<MapPanel>` element's `map` attribute. Without the colon, attributes are interpreted as constant strings just like in plain HTML.
...
The "container variable" `local` only exists to prevent accidential overwriting of predefined component member variables with user-defined ones. It is not a native feature of Vue.js, but was introduced by us a convenience feature to support the high degree of declarative programming in Tivigi.

## 7.2. Roles of the components in the first example
...
## 7.6. The DataMapLayer component
...
## 8.2. The second layer, and how it is different from the first
...
# 9. Tutorial Step 3: Adding a layer tree
...
## 9.4. Bidireactional data binding with `v-model` and `.sync`
...
# 10. Tutorial Step 4: Adding a legend (list of active layers) and feature info tool
...
## 10.1. The `<ActiveLayers>` component
...
## 10.2. The `<FeatureInfoTool>` component
...
# 11. Tutorial Step 5: Adding more features
...
## 11.1. The `<LocationSearch>` component
...
## 11.2. The `<MouseCoordinatesTool>` component
...
## 11.3. The `<SetMapExtentTool>` component
...
## 12.1. Problem: First setting of previously undefined variables in templates trigger component reload
